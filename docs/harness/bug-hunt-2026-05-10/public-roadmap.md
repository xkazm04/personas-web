...
- **Scenario**: Page declares `export const dynamic = "force-static"; export const revalidate = 3600;`. A reader assumes "the roadmap revalidates every hour." It does not — the page shell is static, and `<Roadmap />` renders entirely from static constants (`areas.ts` + `roadmap-phases.ts`). The `revalidate = 3600` value is meaningless here because nothing on the page uses Next.js data caching (no `fetch` in a Server Component, no `unstable_cache`).
- **UPDATE (superseded):** the "fetches `/api/roadmap` on mount" premise this and findings #1/#4/#7 assumed no longer holds — the current `<Roadmap />` performs **no client fetch** (`RoadmapCard.tsx`/`data.ts`/the `index.tsx` fetch effect were removed). The section draws only from static data, so the loader-stuck (#1), 502-parse (#4), and progress-drift (#7) failure modes are moot against today's tree; they remain here as a record of the audited snapshot. The `revalidate = 3600` no-op (this finding) still stands.
