...
| B-12 | Med | `src/proxy.ts` is complete middleware (matcher `/dashboard/:path*`, mobile-UA → `/m/overview`, `prefer-full` escape hatch) but named `proxy.ts`, so Next never loads it — the `/m` redirect **does not run**. | **Wire-or-cut product decision** (activating redirects *all* mobile `/dashboard` visitors to the demo `/m` shell — can't verify blind). Not mechanically changed; needs your call. | 🔵 |
...
| ID | Sev | Finding | Fix | Status |
|---|---|---|---|---|
| D-01 | Med | **Confirmed real:** no `--color-surface` is registered anywhere (no `tailwind.config`, absent from `@theme`), yet `bg-surface/95` is used in **20 modal/toast/overlay components** (`Modal`, `ConfirmDialog`, `UndoToast`, tour cards…) — so `bg-surface` is a non-resolving utility producing no fill (they currently render as blur+border glass, not the intended ~95%-opaque surface). | Needs design intent: register `--color-surface` per the 14 theme variants (must be a near-solid panel color, *not* the 0.02-alpha `--card-bg`) + visual QA. **Not blind-fixed** — a wrong value regresses 20 surfaces × 14 themes. | 🔵 |
...
| ID | Sev | Finding | Fix | Status |
|---|---|---|---|---|
| G-01 | High | Demo signin/`enterDemo` reachable in prod (no env flag); `AuthGuard` is client-only, gates on `isAuthenticated`, no role/server guard. | Gate demo entry; treat dashboard as non-authoritative. | 🔵 |
| G-02 | High | Vote stuffing: client-minted `voterId`, no `UNIQUE(feature_id,voter_id)`, boosts trust client `weight`/`tierValue` (no payment verify), per-process rate-limit keyed by spoofable/"unknown" IP. | Server-bound identity + DB constraint + verified boosts. | 🔵 |
| G-03 | High | Sentry PII scrubber: `scrubData` only deletes `SENSITIVE_FIELDS` keys → off-list keys (`email`/`workspace_id`) survive in `contexts`/`extra`/`tags` unless value matches regex; `user.id` scrubbed-not-dropped; depth-6 cap; direct `captureException` bypasses scrub. | Scrub by value across all event sections; prefer `captureExceptionScrubbed`. | 🔵 |
| G-04 | Low | `STATS_ADMIN_TOKEN` absent from `.env.example`; constant-time check length-leaks via early `length` return. | Document the var; fix the compare. | 🔴 |
