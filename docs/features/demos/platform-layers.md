...
- `ConnectionPillar` (`ConnectionPillar.tsx:6`) is the visual pillar: a 1px-wide dashed gradient column (masked with a repeating-linear-gradient) blending `from`→`to` brand colors, plus a `motion.div` dot that loops `top: 0%→100%` every 2 s to suggest data flowing down the connection - only while the stack's loop-gate `run` (threaded from `index.tsx` through `LayerConnection`) is true; otherwise `loopTransition` rests it at 50%.
...
## Conventions & gotchas
- **One loop gate for the stack.** `index.tsx` calls `useLoopGate` (`src/hooks/useLoopGate.ts`) once on the container: `still` (the SSR-safe, live `useStillMotion` answer) becomes `Layer`'s `prefersReducedMotion`, which zeroes the static `rotateX` 3D tilt (`Layer.tsx:68`); `run` stops `ConnectionPillar`'s flow dots for reduced motion, a hidden tab, or a scrolled-past stack. The scroll-driven spread spring and the `StackLabels` stagger are scroll-linked / one-shot, not loops, and are not gated.
