...
- **Performance View** (the `lanes` variant) — one horizontal lane per producer→consumer route (e.g. Gmail→Jira), each showing a queue-depth fill bar, a travelling delivery dot, throughput (`msgs/s`), waiting count, and the delivery time in ms (the simulated route latency itself).

The panel header reports a running "being sent / waiting" count (the snapshot's internal `source` tag - `bootstrap` / `mock` - is deliberately not shown), and a legend below shows the average ("typical") delivery time. A "Try it yourself — build a flow" button swaps the whole showcase for the interactive `FlowComposer` (deep link via `#flow=` hash). This is the *marketing* event-bus story on the `/how` page — distinct from the `/dashboard/events` operational monitoring surface.

## How it works
`EventBusShowcase` (`index.tsx`) is the container. It holds the active `variant` (`useState`, default `"swarm"`) and a telemetry `snapshot` seeded synchronously via `createSnapshot("bootstrap", queueRouteSeeds)` in a lazy `useState` initializer (`index.tsx:37`). `useLoopGate` (`src/hooks/useLoopGate.ts`) on the panel ref (`rootMargin: "200px"`) gates the data feed on its `tick` verdict (hidden tab or scrolled past stops it; reduced motion abstains, since numbers are not motion): while `tick` holds, an effect subscribes to a `QueueTelemetryAdapter` (the injectable `telemetryAdapter` prop, else `defaultTelemetryAdapter`) and unsubscribes on exit/unmount (`index.tsx:39-43`). The subscription pushes a fresh snapshot every 1400 ms.

From the snapshot, `index.tsx` derives `laneMetrics` (per-route producer/consumer/depth/latency/eps/color, `useMemo`) and `averageLatency` (rounded mean of route latencies). The panel renders one of two children by `variant`: `<SwarmView uid />` or `<LanesView laneMetrics run />`.

- **SwarmView** (`SwarmView.tsx`) renders a single inline `<svg viewBox="0 0 100 100">`. The featured tool list comes from `swarmTools` (catalogue entries flagged `swarmFeatured`). For each tool, `computeSwarmTiming(i, total)` places it on a radius-35 perimeter circle and precomputes SVG SMIL timing (opacity ramp-in/hold/ramp-out/pause plus a travel pulse toward center `50,50`). Animation is **pure SVG SMIL** (`<animate>` / `<animateTransform>`, `repeatCount="indefinite"`) — no framer-motion, no `requestAnimationFrame`. Its own `useLoopGate` verdict drives `useSvgTimelineGate`, which calls `pauseAnimations()` / `unpauseAnimations()` on the svg; under reduced motion the base opacity and keyframe `values` flatten to a visible rest pose (0.8), so the swarm reads as a still diagram rather than an empty one. Tool icons load from `/tools/{id}.svg`.
- **LanesView** (`LanesView.tsx`) maps `laneMetrics` to framer-motion lane cards, taking every displayed figure from the pure `laneFigures()` (`figures.ts`, pinned by `figures.test.ts`). Each card has a `whileInView` entrance (staggered by index, `viewport once`), a queue-depth fill bar animated to `queueFillPct` (depth / 50, min 8%), and a delivery dot that loops `x: 0% → 2600%` only while the parent's loop-gate `run` is true (`loopTransition(run, …)`; rests at 0% for reduced motion, hidden tab, or offscreen).
...
| File | Role |
| --- | --- |
| `src/components/sections/event-bus-showcase/index.tsx` | Container: variant state, telemetry subscription gated on `useLoopGate`'s `tick`, composer toggle, derived lane metrics + average latency, panel composition |
...
| `src/components/sections/event-bus-showcase/components/SwarmView.tsx` | Radial SVG/SMIL swarm: perimeter nodes, per-node lifecycle timing, particle pulse to center; timeline paused via `useSvgTimelineGate` |
| `src/components/sections/event-bus-showcase/components/LanesView.tsx` | Per-route lane cards: framer-motion fill bars + delivery dot (gated on the loop gate's `run`) |
| `src/components/sections/event-bus-showcase/figures.ts` | Pure `laneFigures()`: sanitised depth/eps, delivery time in ms, queue fill % (tested in `figures.test.ts`) |
...
- **Both loops go through the loop gate.** SMIL is invisible to the `custom-animation/require-animation-gating` lint rule (it only flags `requestAnimationFrame`/`cancelAnimationFrame`) and to the global CSS reduced-motion reset and `.page-hidden` projection (they only set `animation-*`), so the swarm's timeline is paused explicitly through `useSvgTimelineGate`. `src/lib/motion/loop-gate.test.ts` is the checker the lint rule cannot be: it fails if a file here keeps a `repeatCount="indefinite"` loop without pausing it, or reintroduces a bare `repeat: Infinity`.
