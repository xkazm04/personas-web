...
- **Counts are derived, not hardcoded — but the prose still says "50+".** The hero, pills, and ghost watermark use `connectors.length` (125) and `categories.length` (18) live. However `layout.tsx` metadata and the OG description say "50+ integrations" — stale copy that undersells the real count. Flag if updating marketing numbers.
