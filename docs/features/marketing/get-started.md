...
- **Animation gating:** the auto-advance cycle is gated (via `useAutoCycle` → `useReducedMotion`). `DownloadVisual`'s bobbing icon and `CreateVisual`'s blinking cursor read `useStillMotion` (not framer's SSR-unsafe `useReducedMotion`) and, being ambient loops, also `usePageVisibility`: when either says still, `animate` switches to a rest pose with a zero-duration transition, so a running loop is replaced and the DOM shape never changes. `DownloadStepGrid` (stagger + glow pulse) and `FooterLinkColumn` read `useStillMotion` too; `conversionStillMotion.test.ts` (in `get-started/`) pins all four with a comments-stripped source scan. `WorkVisual`'s pulsing dot and `DownloadCTA`'s orbit ring are CSS keyframe loops gated in the stylesheet, not in JS (so their className and DOM never depend on the preference): the dot is `motion-safe:animate-pulse` (Tailwind's `@media (prefers-reduced-motion: no-preference)`), `.animate-spin-slow` is declared only inside the no-preference block of `src/app/globals.css` and forced to `animation: none` under reduce, and `.page-hidden *` pauses every keyframe animation on a hidden tab (`usePageVisibility` toggles the class; `AnimationPauseObserver` in `PageShell` mounts it). `conversionStillMotion.test.ts` pins all three. The `custom-animation/require-animation-gating` lint rule only fires on `requestAnimationFrame`/`cancelAnimationFrame`, so framer-motion/CSS loops slip past it.
