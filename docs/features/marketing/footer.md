...
- **Animation gating:** `FooterLinkColumn`'s mobile accordion (`AnimatePresence` + `motion.ul` height/opacity) reads `useStillMotion` and drops its transition to `{ duration: 0 }` under reduced motion; the panel's markup is the same either way. It used framer's `useReducedMotion`, which answers `null` on the server and never updates; `get-started/conversionStillMotion.test.ts` now pins the swap.
