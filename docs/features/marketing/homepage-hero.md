...
| `src/components/sections/hero/AgentConstellation.tsx` | Orbiting agent nodes layered into the command-center illustration |
...
- **Env:** `NEXT_PUBLIC_GITHUB_URL` (fallback `https://github.com/personas-ai`) read directly; `NEXT_PUBLIC_DOWNLOAD_URL` and `NEXT_PUBLIC_APP_VERSION` only through `src/lib/release.ts` (`DOWNLOAD_PLAN`, `SITE_VERSION`) — a source scan in `release.test.ts` fails if a component reads them itself.

## Conventions & gotchas
- **Dead hero strings were purged.** `HeroTransition.tsx` (and its `heroTransition` i18n namespace) plus eight unused `t.hero.*` keys (`title`, `subtitle`, `cta`, `mode1`, `mode4`, `executions`, `phases`, `joinWaitlist`) are gone from the interface and all 14 locales. Before re-adding a hero key, confirm a render path exists.
...
- **Raw colors are tokenized.** The hero's `rgba(...)` shadows/strokes (`HeroClient.tsx`, `HeroStatRow.tsx`, `CommandCenterIllustration.tsx`, `PrimaryCTA.tsx`) now go through the `color-mix(in srgb, var(--token) N%, transparent)` idiom, so they follow the active theme. Keep that form when adding SVG strokes or glow shadows — a literal `rgba()` here is a theme bug. `bg-white/2`, `bg-white/3`, `bg-white/[0.02]` opacities are used heavily — fine for surfaces, but watch any text opacities below `/60`.
- **Animation gating is correctly applied** in `HeroClient.tsx` and `CommandCenterIllustration.tsx` (both call `useReducedMotion` and short-circuit loops/transforms). `CommandCenterIllustration` additionally renders static fallbacks for every loop rather than just dropping them. Keep this pattern when extending the illustration.
