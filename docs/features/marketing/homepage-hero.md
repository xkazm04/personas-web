...
- **Token violations (raw colors).** `HeroClient.tsx` + `CommandCenterIllustration.tsx` use literal `rgba(...)` and `rgba(255,255,255,…)` strokes/shadows instead of semantic tokens. Some are unavoidable for SVG gradient stops. `bg-white/2`, `bg-white/3`, `bg-white/[0.02]` opacities are used heavily — fine for surfaces, but watch any text opacities below `/60`.
