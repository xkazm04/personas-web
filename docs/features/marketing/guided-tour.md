...
It auto-advances (audio `ended` event, or a dwell timer for silent steps) and it is designed never to strand anyone: a narration clip that stalls mid-sentence is caught by a watchdog ceiling and the tour moves on, and a step whose spotlight anchor never appears drops the dim overlay so the page stays readable while the narration continues. The visitor can also pause, scrub via progress dots, step with arrow keys, jump between chapters, adjust narration volume (persisted), or press Escape to exit. A first-visit pulse ring on the launcher and a `?tour=1` URL param (used by the bridges) auto-start the tour; a `localStorage` "seen" flag suppresses the pulse afterward. Reduced-motion users get snaps instead of tweens and a static avatar.
...
| `src/hooks/useDialogFocusTrap.ts` | Shared modal focus trap (Tab cycle, initial focus, focus restore). Promoted here from `src/components/dashboard/event-detail-drawer/`; used by the three tour cards and `EventDetailDrawer` |
...
| `src/components/tour/TourVolumeControl.tsx` | Volume slider bound to context (default **0.25** — the file's own docblock claims "Default 50%", which is wrong; `useTourVolume.ts` is the authority) |
...
- **Audio cleanup is load-bearing.** Each new clip creates a new `Audio` + `MediaElementAudioSourceNode`; the effect cleanup pauses the element, disconnects the source, and clears the `ended`/`error`/watchdog timers. The shared `AudioContext` is reused across clips for the provider's lifetime and **closed on unmount** (a separate mount-scoped effect) — without that, repeated tour runs exhaust Chromium's ~6-context-per-document cap and narration silently dies. `AvatarCountdown` closes its own separate context. A given media element can only be wrapped by `createMediaElementSource` once, which is why a fresh element is made per clip rather than reusing one; reusing would throw.
- **The three cards are real modals now.** `TourIntroCard`, `TourCaptionCard` and `TourBridgeCard` all declare `role="dialog"`/`aria-modal="true"`, so they all use the shared `useDialogFocusTrap` (`src/hooks/useDialogFocusTrap.ts`) — one implementation, promoted out of the event-detail drawer rather than forked. Each card passes the same option block: `initialFocusSelector: "[data-tour-focus]"` (the card's *primary* control — Begin / Play-Pause / Continue — since the tour has no `[data-drawer-close]`), `escape: false` (Escape belongs to `useTourKeyboard`; both listeners sit on `window` in the bubble phase, so `stopPropagation` would not stop the other and exit would fire twice), `restoreFocusSelector: "[data-tour-launcher]"` (the launcher unmounts during a run, so the node captured at open is detached by close — the trap looks up the re-mounted one), and `deferToMovedFocus: true` (intro hands over to caption while still animating out; without this its cleanup would yank focus back off the card that just took it). Every default of the hook reproduces the drawer's original behaviour exactly, so `EventDetailDrawer` passes no options at all. If you add a control to a card, keep exactly one `data-tour-focus` in it.
- **No-strand backstops (two, both instrumented).** `ended` is the *only* event that advances an audio-bearing step, so `useTourAudio` arms a **stall ceiling** per clip: `max(clip duration once known, step.dwellMs) + STALL_SLACK_MS (8s)`. It is armed at clip creation (which also covers a refused autoplay, a path that fires no media event at all), re-armed on `loadedmetadata`/`durationchange`/`play`, and cleared on `pause`/`ended`/`error` — so a paused tour never advances behind the visitor and the happy path never sees it fire. Separately, `TourSpotlight`'s rAF loop applies a declared **missed-anchor policy: keep the caption, drop the scrim.** If the selector has not resolved within `ANCHOR_TIMEOUT_MS` (3s) the cutout fades to opacity 0, so the page reads normally while narration and caption continue; skipping the step was rejected because it would discard narration already playing. It is self-healing (a late anchor restores the cutout) and it also fixes the nastier variant where the cutout kept the *previous* step's rect and confidently highlighted the wrong element. Both paths emit a `category: "tour"` Sentry breadcrumb carrying numbers only (`stepIndex`/`ceilingMs`, `waitedMs`) — no SENSITIVE_FIELDS key and no user-derived value, so nothing depends on `sentry-pii.ts` to be safe. Covered by `e2e/tour.spec.ts` → "Guided tour — no-strand backstops".
...
- **Lazy-mount timing:** spotlight/scroll/`data-tour-active` all *poll* because target diagrams hydrate lazily; a step whose diagram never mounts (wrong `scrollTarget` or removed `data-tour-diagram`) will scroll-and-wait, then after 3s drop the scrim and carry on narrating without ever spotlighting — visible in Sentry as the missed-anchor breadcrumb, not as a broken screen. Verify both the wrapper id and the diagram anchor when adding steps.
