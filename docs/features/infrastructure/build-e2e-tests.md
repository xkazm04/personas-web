...
| `e2e/{blog,connections,templates,playground,legal,roadmap}.spec.ts` | Per-route content + filter/search/tab coverage (roadmap reads its copy from `en.ts`) |
| `e2e/landing-hash-arrival.spec.ts` | `/#download` cold load, hero CTA and in-page `#download` link: lazy section mounts, lands in view, heading focused |
...
- **Coverage gaps that remain.** `dashboard-demo.spec.ts` smoke-covers `/demo` entry, dashboard home, and the `/m` shell via Try Demo, and `review-undo.spec.ts` drives `/dashboard/reviews` — but the other `/dashboard/*` pages, `/preview` and `/security` have no journey spec, `/how` is covered only by the reduced-motion swarm and hydration checks, and the roadmap spec only asserts render (**the Supabase voting flow is never tested**). Demo mode is in-memory: any `page.goto` resets it, so dashboard specs must re-enter via `/demo` and then navigate client-side.
- **Reduced motion in specs goes through `contextOptions`.** Playwright 1.62 has no `reducedMotion` fixture option: `test.use({ reducedMotion: "reduce" })` is silently ignored. Use `test.use({ contextOptions: { reducedMotion: "reduce" } })` (see `reduced-motion.spec.ts`). (Reduced motion used to make `/how` discard and re-render its streamed `section#event-bus` once during load; that was an app bug in `QualityProvider`/`PageTransition`, now fixed and pinned by `reduced-motion-hydration.spec.ts`, so a one-shot `scrollIntoViewIfNeeded` is enough.)
- **Time-based journeys are bounded, not slept.** The hub spec waits one full `AUTO_CYCLE_MS` + slack to prove nothing advances; the review spec observes the next poll's 300ms "Refreshing..." flash with a `MutationObserver` rather than sleeping 15s. Specs that click near the bottom of the viewport pre-seed `personas-cookie-consent` via `addInitScript` so the banner cannot intercept.
