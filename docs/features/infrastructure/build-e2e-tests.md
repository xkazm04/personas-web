...
| `e2e/{community,compare,download,use-cases}.spec.ts` | Specs whose target routes have **no `page.tsx`** in the current tree (see gotchas) |
| `package.json:19-23` | `build` / `start` / `lint` / `typecheck` / `test:e2e` scripts |
...
## Conventions & gotchas
- **Stale specs against non-existent routes.** `community.spec.ts`, `compare.spec.ts`, `download.spec.ts`, and `use-cases.spec.ts` target `/community`, `/compare`, `/download`, `/use-cases` — none of which have a `page.tsx` under `src/app/` (no route group, no middleware rewrite). As written these would hit Next's 404 and fail their `h1`/`main` assertions. Either the routes were removed or the specs are orphaned; verify before trusting "passing" status.
