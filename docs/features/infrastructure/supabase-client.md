...
## Conventions & gotchas
- **Client reads the anon key; `service_role` is server-only.** Client code only ever reads `NEXT_PUBLIC_SUPABASE_ANON_KEY` (`src/lib/supabase.ts:8`). The privileged `SUPABASE_SERVICE_ROLE_KEY` *does* appear in `src/`, but only inside `import "server-only"` modules — `src/lib/server/env.ts:39` (`hasSupabaseServiceRole()`) and `src/lib/supabase-admin.ts:23` (`getSupabaseAdmin()`) — never carries a `NEXT_PUBLIC_` prefix, and so can never reach the client bundle (Next fails the build if a `server-only` module is imported client-side). Client-side row isolation still relies on RLS over the user session, not on that key. See CLAUDE.md convention 6.
