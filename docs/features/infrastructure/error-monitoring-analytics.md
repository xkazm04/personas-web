...
- **Analytics.** `src/lib/analytics.ts` exposes `trackPageView`, `trackDownloadClick` (`download_click` with `{ platform, placement, outcome }`, from the hero, pricing and download-section CTAs; attributes built by the pure `downloadClickAttributes`), `trackFeatureVote`, `trackFeatureRequest`, `trackFeatureComment`. Each calls `trackEvent` (`:16`), which emits `Sentry.metrics.count(name, 1, { attributes })` only when consent is `"all"`; otherwise it queues (cap 50, FIFO drop). `flushAnalyticsQueue` (`:27`) drains the queue once consent is granted.
