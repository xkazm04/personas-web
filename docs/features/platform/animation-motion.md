...
| File | Role |
| --- | --- |
| `src/contexts/QualityContext.tsx` | Adaptive `high/medium/low` tier from p90 frame time; `useQualityTier` / `useReducedMotionPreference` (= `useStillMotion`) |
...
| `src/lib/motion/loop-gate.ts` | Pure loop-gate contract: `LOOP_DECIDERS`, `resolveLoop`, `resolveTicker` (data feeds: preference abstains), `loopTransition` (the one place `repeat: Infinity` is written for the visualizers) |
| `src/hooks/useLoopGate.ts` | `useLoopGate(ref, { userStopped, rootMargin })` -> `{ run, tick, still, vetoedBy }` from `useStillMotion` + `usePageVisibility` + `useIsVisible`; `useSvgTimelineGate` pauses a SMIL timeline on `run` |
...
## Conventions & gotchas
- **Gating is mandatory** for any rAF/canvas/GPU-intensive component: call **`useStillMotion()`** (`src/hooks/useStillMotion.ts`) or `useReducedMotionPreference()` (QualityContext), plus `useQualityTier()` to scale complexity. The contract is spelled out in `animations.ts:3-37`.
- **Do not `return null` (or swap elements) on the preference in a component that server-renders.** framer's `useReducedMotion` answers `null` on the server and the real value on the client's first render, so branching markup on it desyncs hydration and React discards the subtree — the most work possible for the visitors who asked for less. `useStillMotion` wraps it in `useSyncExternalStore` with a `getServerSnapshot`, which React also uses for the hydrating render. Gate the `animate`/`variants`/`transition` props instead so DOM shape stays constant. Components reached only through `dynamic(..., { ssr: false })` never server-render and are exempt.
- **Nothing above a route's `loading.tsx` boundary may change on the preference.** Every route streams inside that Suspense boundary, below `QualityProvider` and `PageTransition`. A context change reaching it while it is still pending (a provider value flipped from an effect, or a framer `motion.*` whose `initial`/`animate` change - framer re-publishes those two as MotionContext) makes React discard the streamed HTML and client-render the page, with no hydration error and no overlay. Even `useStillMotion` is not enough there, because its correction lands one commit after hydration. So `QualityProvider` keeps the preference out of its value and memoizes `{ tier }`, and `PageTransition` gates in CSS only (`motion-reduce:opacity-100! motion-reduce:transform-none!`). Pinned by `src/contexts/reducedMotionHydration.test.ts` (source scan) and `e2e/reduced-motion-hydration.spec.ts` (counts `section#event-bus` discards on `/how`).
- **The `custom-animation/require-animation-gating` lint rule has a blind spot.** It only fires when a file textually uses `requestAnimationFrame`/`cancelAnimationFrame` without importing `useReducedMotion`. It does **not** catch framer-only motion (`motion.*`, `animate`, `repeat: Infinity`) or CSS-keyframe components — those can ship an ungated infinite loop and pass lint. Gate them manually via `useStillMotion` (plus `usePageVisibility`/`useIsVisible` for ambient loops), or through `useLoopGate` + `loopTransition`, which the visualizer sections are held to by `src/lib/motion/loop-gate.test.ts` (source scan). SMIL `repeatCount="indefinite"` is reached by neither lint nor the CSS reset - pause it with `useSvgTimelineGate`. Also note `particleHostRegistry.ts:1` disables the rule with a file-level comment because its React wrapper does the gating; if you add a new entry point that doesn't go through `ParticleHost`, that exemption no longer holds.
