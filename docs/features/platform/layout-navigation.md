...
**Navbar** — `Navbar.tsx` is a `motion.header` that toggles a blurred-glass background once `scrollY > 40` (via `useScroll` + `useMotionValueEvent`). It delegates to `DesktopNav` (the `lg:flex` pill bar + dashboard/download cluster) and `MobilePanel` (the `lg:hidden` slide-in), sharing open/close state through `useMobileMenu`. The hamburger button toggles `mobileOpen` and exposes `aria-expanded`. Both navs' Download CTA calls `openDownload`: when the release plan (`DOWNLOAD_PLAN` from `src/lib/release.ts`, the same rule `/api/download` enforces) says the visitor's detected platform downloads, it reports `download_click` (`placement: "navbar"`, `src/lib/analytics.ts`) and goes to `/api/download`; otherwise it opens the lazily loaded `WaitlistModal` for that platform.
...
**Page shells** — `PageShell` wraps page content in `SectionObserverProvider` (scroll-spy over the section ids derived from `scrollMapItems`) and `TourProvider`, mounting `AnimationPauseObserver`, `ParticleHost`, `ScrollMap`, and `TourOverlay` around a single `<main id="main-content">`. `ScrollMap` clicks resolve through `resolveLandingAddress` and hand off to the shared `startArrival` runner (see below), so a dot whose section has not mounted yet still lands on it. `InfoPageLayout` composes `Navbar` + `PageShell` + an optional `SectionBreadcrumb`, `MobilePageTOC`, a fixed-navbar spacer, an optional `TourLauncher`, and `Footer`.

**Home address space & hash arrival** — `src/lib/landing-address.ts` is the one resolver for the home page's addresses: every `LANDING_SECTIONS` id, plus the external wrapper ids as aliases (`download-section` → `download`, `tools` → `use-cases`, `playground` → `playground-split`). An address carries the always-present wrapper selector (`[data-scroll-anchor="<id>"]`, or `#hero`) and the inner section id (`pipelines` → `orchestration-hub`, `vision` → `vision-grid`). Hashes outside that space (`#flow=…`, `#main-content`) resolve to `null` and are left alone. `useHashArrival` (mounted through `<LandingHashArrival />` in `src/app/page.tsx`) runs the pure `step()` table on a 100 ms tick on load and on `hashchange`: `seeking` scrolls the wrapper so `LazyMount` fires, `landed` scrolls to the section and focuses its heading, then re-asserts for `REASSERT_MS` if the layout shifts; the reader's own wheel/touch/scroll-key/pointer input cancels it, and it gives up after `ARRIVAL_BUDGET_MS`. A cold arrival jumps (`instant`), like a native fragment load; the landing scroll is smooth unless `useStillMotion` says otherwise.
...
| `src/lib/landing-address.ts` | Home address resolver + pure arrival transition table (unit-tested, with the `/#x` link coverage guard) |
| `src/hooks/useHashArrival.ts` | `startArrival` runner (shared with `ScrollMap`) + `useHashArrival` (load + `hashchange`) |
| `src/components/LandingHashArrival.tsx` | Client leaf that mounts `useHashArrival` on the server-rendered home page |
...
- **`PageTransition` reads no motion preference in JS.** It wraps every route directly above the root `loading.tsx` Suspense boundary, so its `initial`/`animate` (which framer re-publishes as MotionContext) must stay constant: flipping them after hydration for reduced-motion visitors made React discard the still-streaming page and client-render it. Reduced motion is `motion-reduce:opacity-100! motion-reduce:transform-none!` on the wrapper, which overrides framer's inline styles from the first paint (see [Animation & Motion System](animation-motion.md)).
...
- **Hash arrival ignores its own scrolling.** Cancellation listens only to reader-intent events (`isUserScrollIntent`: wheel, touchmove, pointerdown, scroll keys), never to `scroll`, because the runner's own `scrollIntoView` fires `scroll` and would cancel the arrival on its first move. A Next `<Link>` to a hash on the page you are already on uses `pushState`, which fires no `hashchange`, so it is not handled; plain `<a href="#x">` is.
