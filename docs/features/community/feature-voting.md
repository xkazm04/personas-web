...
- **React 19 safety:** `FeatureVoteCard` correctly uses the prev-state reset pattern instead of `setState`-in-effect; voterId is mutated through a `ref`, not state.
- **Reduced-motion gating.** Every framer interaction in the section gates on `useReducedMotion()` (`FeatureVoteCard` hover-lift + comment expand, `FeatureBoostButton` tier popover, `CommentThread` reply expand, `CustomFeatureRequest` success/error fades, the `FeatureVotingSummary` skeleton pulse): when reduced, the element renders its final state with no motion; non-reduced users are unchanged. CSS `hover:scale-*` on the Ko-fi tier links is left as a lightweight, exempt transition. The section-entrance `fadeUp`/`staggerContainer` reveals are inherited from `SectionWrapper` (site-wide convention, ungated) — the `FeatureVotingGrid` card grid additionally self-drives its own `whileInView` so a post-scroll load still reveals.
