...
- **Reduced motion:** `RevealTile` reads `useReducedMotion()` and, when true, sets the final `clipPath`/seam `left` statically (no animation). Preserve this branch when editing tiles. `PhaseCardStrip` also gates its scroll-RAF and mobile-peek effect behind `useReducedMotion` (required by `custom-animation/require-animation-gating`).
