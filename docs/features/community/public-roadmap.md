...
`<Roadmap />` (`sections/roadmap/index.tsx`) is a **server component**: the section is visually static, so it renders on the server (with thin client wrappers handling motion) and reads the derived area counts at build time (`AREA_COUNTS` from `roadmap-area-counts.ts`), passing them to `<RoadmapAreas counts={…} />` as plain data. That keeps the heavy templates/connectors catalogs entirely out of the client bundle. It renders a `RoadmapIntro` (a thin client wrapper over `SectionIntro`) + `<RoadmapProgress />` + `<RoadmapAreas />`, all of which read localized copy from the `roadmapSection` i18n namespace:
...
- **The section is fully localized via the `roadmapSection` i18n namespace** (all 14 locales). The intro, `RoadmapProgress` copy (including the `noneDone`/`firstDone`/`rangeDone` special-cases and the `toGoOne`/`toGoOther` plural split), every area title/caption, bar label, and detail string flow through `t.roadmapSection`. **Derived numbers stay derived** — `buildAreas` interpolates the build-time counts into localized templates (`{n}`, `{total}`, `{count}`, `{completed}`) with `String(x).replace(...)`; never hardcode a count into a locale string. Because `index.tsx` is a server component and `useTranslation` is client-only, the intro copy is resolved in the `RoadmapIntro` client wrapper, not on the server. Platform/proper-noun tokens (`Windows`, `macOS`, `Linux`, `Web`, `DevOps` + the template-gallery category identifiers, `RTL`, `SSO`) are intentionally kept in English across locales. **Page `metadata` in `app/roadmap/page.tsx` stays English-hardcoded** — that matches every other static page in the repo (no `metadata` i18n mechanism exists); do not invent one here.
