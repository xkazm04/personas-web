...
## How it works
- **Layout** (`src/app/guide/layout.tsx`) renders `Navbar` + the always-present `GuideSidebar` + a `<main id="main-content">`; the sidebar is `sticky` on desktop and a slide-in `role="dialog"` drawer under `lg`. `<main>` deliberately has **no** `overflow` — the window is the scroll container, which is what lets `position: sticky` work inside it. All navbar-relative offsets come from `src/components/guide/guide-chrome.ts`.
...
| `src/app/guide/[category]/not-found.tsx` | Unknown-category 404 (localized strings → `GuideNotFound`) |
...
| `src/app/guide/[category]/[topic]/not-found.tsx` | Unknown-topic 404 (localized strings → `GuideNotFound`) |
| `src/components/guide/GuideNotFound.tsx` | Shared 404 body for both guide 404 routes — renders inside the guide shell (no own Navbar/Footer) |
...
| `src/components/guide/guide-chrome.ts` | Single source of truth for the navbar-relative offsets of every fixed guide bar |
| `src/components/guide/CategoryGlyphArt.tsx` | Fallback category artwork (brand rings + lucide glyph) for categories with no painted illustration |
| `src/data/guide/content/companion.ts` | Markdown bodies for the `companion` (Athena) category |
...
## Conventions & gotchas
- **Hardcoded English (lint/i18n violation):** despite the all-strings-in-`en.ts` rule, large parts of this surface ship literal English in JSX — e.g. `TopicView.tsx` ("Skip to content", "min read", "Ready to try this yourself?", "Download free", "Browse templates", "Previous"/"Next"/"Back to", breadcrumb "Guide"), `CategoryTopics.tsx` (`Filter N topics...`, "results found", empty state), `[category]/page.tsx` ("Back to Guide", `N topic(s)`), `RelatedTopics.tsx` ("Related Topics"), `ModuleBadge.tsx` ("In app", "Find in app", the instructional sentence), `GuideSidebarContent.tsx` ("Search topics...", dev-only `title`), and `SearchResultsPopover.tsx` ("No topics found", "Searching…", "N results"). Only `TopicView`'s TOC label (`t.pageNav.onThisPage`), the mobile TOC, the two 404 routes (`t.guide.categoryNotFound` / `t.guide.topicNotFound` / `t.guide.guideHub`), and the hub use translations. Adding/changing copy here should migrate it into `en.ts` + all 13 locales.
- **404 routes render inside the guide layout:** Next mounts `not-found.tsx` within the nearest layout, and `guide/layout.tsx` already supplies `Navbar` + `GuideSidebar`. The 404 bodies therefore render page content only (via `GuideNotFound`) — adding a `Navbar`/`Footer` there stacks a second header.
- **Hidden topics never get linked:** prev/next in `[category]/[topic]/page.tsx` and `getRelatedTopics` both filter through `isTopicVisible`, so a `devOnly` topic hidden in production is not reachable from a neighbour link (which would otherwise `notFound()`).
...
- **Category artwork is never optional:** `GUIDE_ILLUSTRATIONS` only covers the ten categories with painted art; `companion` has none. Both the hub card and the category banner keep the artwork slot and fall back to `CategoryGlyphArt`, which resolves `GuideCategory.icon` (a lucide component name) through an explicit map. Adding a real illustration pair under `public/imgs/guide/` + an entry in `illustrations.ts` upgrades it with no component change.
- **Guide counts are derived:** `layout.tsx` builds its `metadata.description` from `GUIDE_CATEGORIES.length` / `GUIDE_TOPICS.length` — it previously claimed "10 categories" after the 11th shipped. Category labels/descriptions live in `t.guide.categories` / `t.guide.categoryDescriptions`; a category missing there falls through to the raw English `category.name` in **all** locales, so every new category needs both keys in all 14 bundles.
- **Reading-chrome band (`guide-chrome.ts`):** the `Navbar` is content-sized, not class-sized — measured **76px below `lg`, 86px from `lg`** (nav `py-4` + a 44px tap row / 54px nav row). The fixed bars stack as navbar → reading progress (2px) → mobile TOC bar + sidebar trigger (44px, the trigger occupying the `pl-16` lane the TOC bar reserves) → content at 122px. Never hardcode an offset here; import the class constants. The desktop on-this-page TOC is `sticky` at 102px, which only works because `<main>` has no `overflow` (an `overflow-y-auto` on an unconstrained-height element makes it the scrollport without ever scrolling, and every `sticky` inside it goes inert).
- **Sidebar drawer is a real dialog:** `GuideSidebar` composes `useFocusTrap` (`src/hooks/useFocusTrap.ts`) + `lockBodyScroll`/`unlockBodyScroll` + an Escape handler, so `role="dialog" aria-modal="true"` is backed by initial focus, Tab cycling, focus restore, and a scroll lock. Reuse those primitives rather than re-implementing.
