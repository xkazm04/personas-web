...
- **Section-header comments in `topics.ts` are only roughly trustworthy.** The `// ─── Getting Started (12)` style counters were refreshed for the categories last edited (Getting Started 12, Monitoring 15, Companion 8), but others can still drift (e.g. Credentials says 10 but has 7). Treat them as hints, not a source of truth — count the records.
