...
1. **Categories** (`categories.ts`) — 10 `GuideCategory` records: `id`, `name`, `description`, Lucide `icon` name, hex `color`, and an optional default `mode`. Categories flagged `mode: "power"` (triggers, pipelines, memories, monitoring, testing, deployment) only surface under the power-mode filter.
2. **Topics** (`topics.ts`) — 100 `GuideTopic` records, each with `id`, `categoryId`, `title`, `description`, `tags[]`, optional `mode`/`devOnly`, and optional `coverage` metadata (screenshot recipe path, review timestamps, watched desktop files for the drift detector). Topics are the searchable/navigable index; they carry **no body**.
3. **Content** (`content/<category>.ts`) — one file per category exporting `content: Record<topicId, markdownString>`. `content/index.ts` spreads all ten into a single `GUIDE_CONTENT` map. Bodies are template-literal Markdown with custom fenced directives (`:::steps`, `:::cards`, `:::compare`, `:::cli`, `:::keys`, `:::usecases`, `:::callout-stack`, `:::code-compare`, `:::warning`, `:::tip`, `:::info`) that the page renderer interprets.
...
| `src/data/guide/categories.ts` | 10 category records (id, name, icon, color, mode) |
| `src/data/guide/topics.ts` | 100 topic records (the searchable/navigable index; no bodies) |
| `src/data/guide/content/index.ts` | Spreads 10 category files into the `GUIDE_CONTENT` topicId→Markdown map |
...
## Conventions & gotchas
- **Three lists must stay in lockstep.** `topics.ts` (id), every `content/<cat>.ts` (key), and `TOPIC_MODULE_MAP` (key) are all keyed by `topicId`. Currently a clean 100/100/100 match with no orphans, but only one of the three drift cases is guarded: `guide-utils.ts:73` throws at module load if a topic references an unknown `categoryId`. **A topic with no matching `content` key is *not* caught here — it fails at runtime with `notFound()` (`page.tsx:105`), and an orphan `content` key (body with no topic) is never surfaced at all.** Add/remove a topic in all three places together.
- **Stale section-header comments in `topics.ts`.** The `// ─── Getting Started (10)` style counters are out of date: Getting Started says 10 but has 8, Credentials says 10 but has 7, Monitoring says 10 but has 11. Real-count drift only — harmless to the app, misleading to a reader. Don't trust the parenthetical counts.
