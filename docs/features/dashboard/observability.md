...
**Activity data — `ActivityMetricsView.tsx` + `activity-view/`.** `useActivityMetrics()` (SWR over the standalone `getActivityMetrics` mock fetcher) returns `{ athenaUsage, valueRollup, athenaActionMix, athenaLedger }`. The view owns a local `compare` state driving a `CompareToggle`, then renders `AthenaUsageCard` (card chrome + totals; the stacked `AreaChart` of invoke/recall/fallback with a dashed `prevTotal` overlay when comparing lives in `@/components/dashboard/AthenaUsageChart` and is `dynamic(..., { ssr: false })` — splitting it took 406 KB out of this route's first load) and `ValueRollupCard` (rate + cost-per-value with `trendColor`-coloured deltas, and an outcome bar), then the Athena lane row: `AthenaActionMixCard` (op-grammar cost bars from `MOCK_ATHENA_ACTION_MIX` — slugs are `AthenaOpAction` machine tokens labeled via `observabilityPage.athenaOps.*`, never shown raw) and `AthenaSpendLane` (turn-ledger totals from `MOCK_ATHENA_LEDGER`; avg/turn and the vs-fleet share are derived in the component; the ratio bar clamps at 100% and floors at 2% for visibility). Demo-only; no real source.

**Performance data — `PerformanceView.tsx`.** Fetches via SWR: `useSWR("observability", api.getObservability, { refreshInterval: 30_000, dedupingInterval: 8_000, revalidateOnFocus: false, keepPreviousData: true })` (`PerformanceView.tsx:28`). From the one payload `{ metrics, dailyMetrics, personaSpend, healthIssues }` it memoizes the chart shapes: `costChartData` (`{date, Cost}`), `execChartData` (`{date, Successes, Failures}`), `spendPieData` (`{name, value, color}`), all keyed by `day.date.slice(5)` (MM-DD). Health issues get a demo-vs-real branch: when the synced list is empty it falls back to `MOCK_HEALTH_ISSUES` **only in demo** (`isDemo`), never in real mode (`PerformanceView.tsx:46`). It derives `filteredHealthIssues`, `severityCounts`, `openIssues`, and `overBudgetPersonas` (cost/budget > `BUDGET_THRESHOLD` = 0.8). `handleRunAnalysis` calls SWR `mutate()` — a genuine re-fetch — and the panel's `healingActive` is SWR's own `isValidating`, so the "analyzing" state lasts exactly as long as the request. While `loading && !metrics` it shows a spinner. A decorative `bg-observability.png` is layered at `opacity-[0.12]` behind the content.

**Overlays are projections of the one daily root — `src/lib/observabilitySeries.ts`.** The daily series `MOCK_DAILY_METRICS` (`src/lib/mockData.ts`, Σ cost 4.82 / Σ runs 47) is the single source for every number on the Performance tab. A second hand-tuned window, `MOCK_DAILY_PRIOR_METRICS` (the 14 days before it, Σ 4.30 / 43), is the "previous period". The pure functions in `observabilitySeries.ts` project both into each overlay: `periodTrend` gives the tiles' "vs last period" pills (`MOCK_OBSERVABILITY_METRICS.costTrend/execTrend/successTrend`, rounded to 1 dp). `previousSeries` builds `MOCK_COST_COMPARE` / `MOCK_EXEC_COMPARE`, keyed by the chart's own `date.slice(5)` (UTC) key. `detectCostAnomalies(daily, 2)` builds `MOCK_COST_ANOMALIES` (days ≥ 2σ above the window mean). `anchorAnnotations` pins `MOCK_ANNOTATIONS` to real x-axis days by offset from today, and `sparklinesFromDaily` builds the tile sparklines. The window holds one genuine spike (day 10, $0.79, z ≈ 3.2), so the anomaly banner names a real day, and the "v2.1 deployed" annotation sits on that same day. The Supabase plane calls the same `periodTrend` through `halvesTrend`: no prior window is synced, so real mode compares the recent half of the daily series with the earlier half, and the Compare toggle stays hidden. `src/lib/observabilityFixtures.test.ts` holds these reconciliations, and `observabilitySeries.test.ts` pins the functions and the Supabase arithmetic.
...
| `src/lib/mock-dashboard-data.ts` | Dashboard fixtures: health issues, latency, success/agents sparklines, and the Performance overlays (compare series, anomalies, annotations) projected from the daily root |
| `src/lib/observabilitySeries.ts` | Pure projections of a `DailyMetric[]` series: `periodTrend`, `halvesTrend`, `sparklinesFromDaily`, `previousSeries`, `detectCostAnomalies`, `anchorAnnotations` (shared by the demo and Supabase planes) |

## Data & state
- **Source:** the daily root in `src/lib/mockData.ts` (`MOCK_DAILY_METRICS`, `MOCK_DAILY_PRIOR_METRICS`, and `MOCK_OBSERVABILITY_METRICS` / `MOCK_PERSONA_SPEND` reconciled against it), the overlays projected from it in `src/lib/mock-dashboard-data.ts` (`MOCK_COST_ANOMALIES`, `MOCK_COST_COMPARE`, `MOCK_EXEC_COMPARE`, `MOCK_ANNOTATIONS`), plus `MOCK_HEALTH_ISSUES`, `MOCK_LATENCY_DATA`, `SPARKLINE_SUCCESS`/`SPARKLINE_AGENTS` and `usage-view/usageViewData.ts` (`MOCK_TOOL_USAGE*`). All served (with simulated `delay`) by `src/lib/mockApi.ts`. Live mode swaps to the orchestrator (`realApi`) or the Supabase mirror (`supabaseApi`) behind the same `api.*` methods.
...
- **Types:** `ObservabilityMetrics`, `DailyMetric`, `PersonaSpend`, `HealthIssue`, `LatencyPoint`, `ToolUsageSummary`, `ToolUsageOverTime`, `ToolUsageByPersona`, `CostAnomaly`, `ChartAnnotation`, `ComparePoint`, `PeriodTrend` (`src/lib/observabilitySeries.ts`, re-exported by `mock-dashboard-data.ts`; the rest in `src/lib/types.ts`). UI types: `SeverityFilter`, `ObservabilityLabels` (`performanceViewTypes.ts`); `BarDatum`/`PieDatum`/`AreaDatum`/`PersonaBarDatum` (`usageChartTypes.ts`); `ChartTooltipEntry`/`ChartTooltipProps` (`chart-theme.tsx`).
...
- **Heads-up: scanned but NOT used by this route (legacy/duplicates).** Several components in `src/components/dashboard/` overlap with the live route but are *not* imported by `/dashboard/observability`:
  - The legacy `ObservabilityCharts.tsx`, `ChartAnnotation.tsx`, `ObservabilityCostChart.tsx` and `ObservabilityExecChart.tsx` have been deleted. The route uses `CostChartWithCompare` / `ExecChartWithCompare` (which inline their own annotation `ReferenceLine`s) and `ObservabilitySpendPieChart`.
...
- **A new Performance overlay is a projection, not a fixture.** Derive it from `MOCK_DAILY_METRICS` through a function in `observabilitySeries.ts` and add a reconciliation case to `observabilityFixtures.test.ts`. Retuning `DAILY_COST` must keep Σ 4.82, because `MOCK_PERSONA_SPEND` and the home Vitals reconcile against it. Annotation dates must be chart x keys, because recharts silently drops a `ReferenceLine` it cannot place.
