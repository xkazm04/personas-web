...
- **Types:** `LeaderboardPersona` (`{ id, name, color, metrics: { reliability, cost, speed, quality, volume }, composite, trend, delta }`) and `LeaderboardTrend = "up" | "down" | "flat"` in `src/lib/mock-dashboard-data.ts:536`. `RadarDatum` (`{ metric, value, benchmark? }`) in `src/components/dashboard/LeaderboardRadarChart.tsx`, re-exported from `LeaderboardRadarCard.tsx` for existing importers. `LeaderboardSortField = "name" | "composite" | "delta"` and `SortDir = "asc" | "desc"` in `leaderboardSort.ts:3`.
