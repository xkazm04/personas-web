...
export default function ModuleTag({ mod }: { mod: OverviewModule }) {
  const Icon = mod.icon;
  return (
    <motion.div
      whileHover={{ scale: 1.03, x: 2 }}
      className="group flex items-start gap-3 rounded-xl border border-foreground/[0.08] bg-foreground/[0.02] px-4 py-3 backdrop-blur-sm transition-colors duration-300 hover:border-foreground/[0.18] hover:bg-foreground/[0.05]"
    >
      <div
