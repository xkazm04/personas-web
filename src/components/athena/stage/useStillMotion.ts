 ...
 * the SSR-safe replacement for framer's `useReducedMotion` on this page.
 *
 * framer's hook reads the media query on the first CLIENT render, but the
 * server has no media query and answers "motion is fine". Any component that
 * branches its MARKUP — or its initial inline styles — on that answer then
 * hydrates against HTML it did not produce, and React responds by throwing the
 * whole tree away and re-rendering the page on the client: the most work
 * possible for precisely the visitors who asked for less. On this page that
 * fired for real — the server sent `<video>` for Athena's avatar while the
 ...
 * server, and the true preference arrives on the very next commit.
 *
