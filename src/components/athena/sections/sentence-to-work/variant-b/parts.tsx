...
/**
 * The atoms this diagram is assembled from.
 *
 ...
 * disappears, so a card solidifying can never move a thread's anchor, which
 * is the whole reason the threads stay attached to what they feed.
 *
...
/**
 * A box in the field. `solid` flips the ghost into the real panel; `waiting`
 * says whether the ghost should be visible at all yet (before Athena has the
 * sentence, an empty slot would give the shape of the answer away).
 */
export function Slot({
  ...
  return (
    <div
      className="absolute"
      style={{ ...rectStyle(rect), transform: tilt ? `rotate(${tilt}deg)` : undefined }}
    >
      <motion.span
 ...
 *  thing here, on a task card and on the answer alike. */
