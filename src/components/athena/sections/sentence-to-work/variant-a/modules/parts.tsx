 ...
 * the order its parts arrive inside that stage, so a stage reads as one
 * gesture rather than a block — every delay stays well under one tick (900ms),
 * which is why layering the whole scene costs the grid nothing.
 *
 * A part MOUNTS when its stage opens: modules are written so that mounting can
 * never move what is already there (columns fill top-down, fixed cells and
 * reserved rows hold their space), which keeps the frozen rect geometry — and
 * therefore the corner brackets — pixel-true throughout.
 *
 ...
 *  group (texture that trails the structure it shares a tick with). */
 ...
 *  beat here, on the request bar, the start control and every finished tile. */
...
/** An accent sweep across a panel the instant something commits on it. Mounts
...
/** One bright beat washing over a row — "this just changed". */
export function Flash({
  ...
  return (
    <motion.span
      ...
      transition={{ duration: 1.1, delay, times: [0, 0.22, 1], ease: "easeOut" }}
      aria-hidden="true"
    />
  );
}
