 ...
 * disappears, so a conversation solidifying can never move a thread's anchor,
 * and the empty place the new fact will land in is held open from the first
 * frame rather than pushing its neighbours aside when it arrives.
 *
 * `Part` owns the order things arrive INSIDE a stage: the tick clock decides
 * which stage a box is at, framer decides the cascade within it, and every
 * cascade finishes well inside one 900ms tick.
 *
 * Anything that MOVES moves on `./Anchor` instead — nothing here animates
 * `left`/`top`, and nothing that carries type is ever scaled.
 */
