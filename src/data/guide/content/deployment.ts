...
Personas agents run in three places: on your local machine (the desktop app's own engine), on a remote orchestrator (cloud-managed by us, or BYOI on your own infrastructure), or on **Anthropic's cloud via Claude Managed Agents** — a zero-infrastructure option where each run executes in an Anthropic-hosted sandbox, billed to your own Anthropic API account. Local is the default and works out of the box; the orchestrator and Claude targets are opt-in. The same agent prompt, tools, and credentials work in any of them — switching is a deployment decision, not a redesign.

The Claude target is the lightest-weight cloud path: paste an Anthropic API key in Deployment → Claude, deploy any agent with one click, and it becomes a versioned agent on Anthropic's platform. Runs, schedules, and triggers route there automatically; connector tools still execute on your desktop with your vault credentials (secrets never enter the sandbox), and services with hosted MCP servers — like GitHub — connect natively with credentials managed in an Anthropic vault. Output, traces, and costs land in the same monitoring views as every other run. Note that schedules fire while the desktop is running; for fully unattended 24/7 operation, the orchestrator remains the right choice.
