...
L'application stocke sa base de données (\`personas.db\`), son coffre d'identifiants, son historique d'exécution et sa configuration localement dans le répertoire de données d'application spécifique à votre OS. Rien n'est téléchargé sauf si vous activez explicitement le déploiement cloud ou utilisez un fournisseur d'IA cloud. Les plugins qui livrent des modèles locaux téléchargent les fichiers de modèle à la première utilisation.
