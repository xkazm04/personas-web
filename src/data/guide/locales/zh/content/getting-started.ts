...
安装后的二进制文件约为 90 MB。如果你启用 Obsidian Brain(向量搜索)等插件,可能会增加占用空间。
...
应用在你操作系统的应用数据目录中本地存储其数据库(\`personas.db\`)、凭证 vault、执行历史和配置。除非你明确启用云部署或使用云 AI 提供商,否则什么都不会上传。包含本地模型的插件会在首次使用时下载模型文件。
