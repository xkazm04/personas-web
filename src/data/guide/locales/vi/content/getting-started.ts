...
Ứng dụng lưu trữ cơ sở dữ liệu của nó (\`personas.db\`), vault credential, lịch sử thực thi và cấu hình cục bộ trong thư mục dữ liệu ứng dụng dành riêng cho hệ điều hành của bạn. Không có gì được tải lên trừ khi bạn cố ý bật triển khai đám mây hoặc sử dụng nhà cung cấp AI đám mây. Plugin ship các mô hình cục bộ tải xuống các tệp mô hình trong lần sử dụng đầu tiên.
