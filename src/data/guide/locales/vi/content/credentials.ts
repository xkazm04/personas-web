...
## Tìm Hiểu Vault Credential

Vault là kho lưu trữ cục bộ được mã hóa nơi mọi credential sống. Về mặt cơ học, đó là một khối được mã hóa AES-256-GCM bên trong cơ sở dữ liệu SQLite của ứng dụng, với chính khóa mã hóa được bao bọc bởi keyring gốc của hệ điều hành. Vault không bao giờ tồn tại trong trạng thái được giải mã hoàn toàn — các credential cá nhân được giải mã từng cái một, trong bộ nhớ, chỉ khi một lần chạy agent cần chúng.

Vault có thể duyệt từ Connections → Credentials. Bạn thấy nhãn của credential, danh mục, trạng thái (khỏe mạnh / sắp hết hạn / hết hạn / bị hỏng) và các phụ thuộc (agent nào sử dụng nó). Các giá trị thô không bao giờ hiển thị sau khi nhập ban đầu — không có công tắc "hiển thị mật khẩu", theo thiết kế.

:::feature
**AES-256-GCM + keyring gốc của hệ điều hành** color=#a855f7
GCM cung cấp cả tính bảo mật và tính toàn vẹn được xác thực — một tệp vault bị giả mạo được phát hiện, không được giải mã âm thầm với rác. Khóa bao bọc sống trong DPAPI (Windows) / Keychain (macOS) / Secret Service (Linux), vì vậy nó được bảo vệ bởi tài khoản người dùng hệ điều hành của bạn, không phải bởi một mật khẩu chính riêng biệt bạn phải gõ.
:::

### Điểm Chính

- **AES-256-GCM cho mỗi credential** — mỗi credential được mã hóa bằng nonce riêng; một sự xâm phạm không lan tỏa
- **Keyring hệ điều hành bao bọc khóa vault** — không có mật khẩu chính riêng biệt để quản lý; bảo vệ đến từ thông tin đăng nhập tài khoản hệ điều hành của bạn
- **Phát hiện giả mạo** — các thẻ xác thực GCM bắt bất kỳ sửa đổi nào; các bản ghi bị giả mạo không thể giải mã với lỗi rõ ràng
- **Thân thiện với kiểm toán** — mỗi quyền truy cập credential được ghi nhật ký với dấu thời gian, agent và ID thực thi; các giá trị thô không bao giờ được ghi nhật ký
- **Ràng buộc với tài khoản hệ điều hành** — sao chép tệp vault sang máy hoặc tài khoản người dùng khác sẽ không làm cho nó có thể sử dụng được

### Cách Hoạt Động

Khi ứng dụng khởi động, nó hỏi keyring hệ điều hành để lấy khóa vault được bao bọc. Keyring giải mã việc bao bọc (sử dụng các bảo vệ ở cấp tài khoản hệ điều hành — DPAPI, Keychain, Secret Service) và trao khóa vault cho tiến trình ứng dụng trong bộ nhớ. Từ đó, ứng dụng có thể giải mã các credential cá nhân theo yêu cầu. Khóa vault không bao giờ được ghi vào đĩa dưới dạng văn bản thuần, và keyring hệ điều hành là nơi duy nhất có thể tạo ra nó.

:::warning
Nếu bạn thay đổi mật khẩu người dùng macOS hoặc Linux của mình, keyring có thể khóa lại khóa bao bọc và nhắc tái xuất nó trong lần truy cập tiếp theo. Điều này là bình thường và có thể khôi phục. Nếu tài khoản hệ điều hành bị xóa hoặc keyring được đặt lại (ví dụ: factory reset), vault trở nên không thể khôi phục — sao lưu bất kỳ bí mật không thể thay thế nào bên ngoài.
:::

:::tip
Bảo mật vault là nhị phân: nó hoặc còn nguyên (tài khoản hệ điều hành hợp lệ, keyring có thể đọc được) hoặc bị hỏng (không thể giải mã). Không có trạng thái trung gian "yếu". Điều quan trọng nhất bạn có thể làm cho bảo mật vault là chạy các phiên bản hệ điều hành hiện đại và sử dụng mã hóa đĩa đầy đủ (BitLocker, FileVault, LUKS) để mô hình mối đe dọa ở cấp thiết bị được giới hạn.
:::
  `,
...
## Xóa Credential An Toàn

Xóa một credential là vĩnh viễn — bản ghi được mã hóa bị xóa khỏi vault và không có cách khôi phục từ bên trong Personas. Trước khi bạn xóa, thẻ credential cho thấy kiểm tra phụ thuộc: mọi agent tham chiếu đến credential, trong slot khả năng nào, với tác động sẽ là gì. Bạn có thể sử dụng hộp thoại xóa để gán lại mỗi agent phụ thuộc cho một credential khác trước khi xác nhận, vì vậy việc xóa thực tế là nguyên tử với việc gán lại.

Đối với các credential OAuth, việc xóa chỉ loại bỏ token được lưu trữ cục bộ — nó không thu hồi truy cập ở phía nhà cung cấp. Nếu bạn cũng muốn thu hồi ở phía nhà cung cấp, hãy làm điều đó trên trang cài đặt bảo mật của nhà cung cấp (một liên kết được cung cấp trong hộp thoại xóa cho các nhà cung cấp chính).

### Điểm Chính

- **Vĩnh viễn và ngay lập tức** — không có hoàn tác; bản ghi được mã hóa bị xóa khi xác nhận
- **Kiểm tra phụ thuộc lên phía trước** — xem mọi agent phụ thuộc trước khi bạn xác nhận
- **Gán lại nội tuyến** — trỏ các agent phụ thuộc vào credential thay thế như một phần của hộp thoại xóa
- **Nhà cung cấp OAuth: chỉ xóa cục bộ theo mặc định** — thu hồi phía nhà cung cấp là một bước riêng biệt (liên kết được cung cấp)
- **An toàn không hoạt động cho các credential đã bị hỏng** — xóa một credential đã hết hạn / bị thu hồi luôn an toàn; không có gì phụ thuộc vào trạng thái chức năng

### Cách Hoạt Động

Hộp thoại xóa đọc cùng đồ thị phụ thuộc như chế độ xem Dependencies. Khi bạn xác nhận, engine trước tiên ghi bất kỳ việc gán lại nào bạn đã chỉ định, sau đó loại bỏ bản ghi credential khỏi vault trong một giao dịch duy nhất. Nếu các việc gán lại không thông qua xác thực (ví dụ: bạn đã cố trỏ vào một credential sai danh mục), việc xóa được hoàn nguyên và không có gì thay đổi.

:::warning
Vĩnh viễn có nghĩa là vĩnh viễn. Bản ghi được mã hóa bị xóa, và nếu bạn không ghi lại bí mật thô ở nơi khác, nó đã mất. Nếu bạn có thể cần credential lại, hãy sao lưu giá trị thô bên ngoài trước khi xóa.
:::

:::tip
Mẫu xoay vòng an toàn nhất là "thêm mới, gán lại tất cả các agent, sau đó xóa cũ". Thêm credential thay thế trước, đi qua bản đồ phụ thuộc để gán lại các agent phụ thuộc từng cái một (hoặc tất cả cùng lúc trong hộp thoại gán lại), xác minh mọi thứ khỏe mạnh, sau đó xóa credential cũ. Trình tự này đảm bảo không có thời gian chết.
:::
  `,
