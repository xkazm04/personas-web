...
// ── Fleet health cascade graph ──────────────────────────────────────
// Nodes are personas; edges encode dependency + cascade propagation risk.

export type CascadeStatus = "healthy" | "degraded" | "failing";
...
export interface CascadeEdge {
  from: string;
  to: string;
  severity: CascadeStatus;
  weight: number; // 0-1 — cascade transmission likelihood
}

export const CASCADE_NODES: CascadeNode[] = [
  { id: "research", label: "ResearchAgent", status: "healthy", x: 0.15, y: 0.3, score: 95 },
  { id: "code", label: "CodeReviewer", status: "healthy", x: 0.15, y: 0.72, score: 91 },
  { id: "data", label: "DataProcessor", status: "degraded", x: 0.5, y: 0.18, score: 68 },
  { id: "report", label: "ReportGen", status: "healthy", x: 0.5, y: 0.82, score: 88 },
  { id: "notify", label: "NotifyBot", status: "failing", x: 0.85, y: 0.5, score: 42 },
];

export const CASCADE_EDGES: CascadeEdge[] = [
  { from: "research", to: "data", severity: "degraded", weight: 0.42 },
  { from: "research", to: "notify", severity: "failing", weight: 0.78 },
  { from: "code", to: "notify", severity: "failing", weight: 0.65 },
  { from: "code", to: "report", severity: "healthy", weight: 0.25 },
  { from: "data", to: "report", severity: "degraded", weight: 0.55 },
  { from: "report", to: "notify", severity: "failing", weight: 0.85 },
];

// ── Burn-rate projection ────────────────────────────────────────────
// Past actual spend + forward projection toward a monthly budget target.
...
export const BURN_RATE_TARGET_USD = 400;

export const MOCK_BURN_RATE: BurnRatePoint[] = (() => {
  const rng = seededRandom(321);
  const past = 14;
  const future = 14;
  const points: BurnRatePoint[] = [];
  let cumulative = 0;
  const dailyBase = 14 + rng() * 3; // ~14-17
  for (let i = 0; i < past; i++) {
    const d = new Date();
    d.setDate(d.getDate() - (past - 1 - i));
    cumulative += dailyBase + (rng() - 0.3) * 4 + i * 0.25; // slight acceleration
    points.push({
      date: `${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`,
  ...
  const lastActual = points[points.length - 1].actual ?? 0;
  let projCumulative = lastActual;
  for (let i = 0; i < future; i++) {
    const d = new Date();
    d.setDate(d.getDate() + i + 1);
    projCumulative += dailyBase + i * 0.35;
    points.push({
      date: `${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`,
      actual: null,
...
// ── Predictive alerts ───────────────────────────────────────────────
// Forward-looking incidents ranked by probability + impact.
...
export const MOCK_PREDICTIVE_ALERTS: PredictiveAlert[] = [
  {
    id: "pa_1",
    title: "NotifyBot likely to hit Slack rate limit",
    detail: "Burn rate 4.2× normal; current trajectory breaches tier-2 cap in ~6h.",
    persona: "NotifyBot",
    probability: 0.87,
    etaHours: 6,
    severity: "critical",
  ...
  {
    id: "pa_2",
    title: "DataProcessor OOM projected under Monday peak",
    detail: "Historic Monday load shows 1.8× Friday peak; current heap headroom is 19%.",
    persona: "DataProcessor",
    probability: 0.62,
    etaHours: 36,
    severity: "warning",
  ...
  {
    id: "pa_3",
    title: "CodeReviewer queue depth climbing",
    detail: "Queue has grown 22% week-over-week while drain rate stayed flat.",
    persona: "CodeReviewer",
    probability: 0.48,
    etaHours: 72,
    severity: "warning",
  ...
  {
    id: "pa_4",
    title: "ReportGen budget likely to overshoot by ~8%",
    detail: "Cumulative spend 87% of monthly budget with 11 days left; trendline extrapolates 108%.",
    persona: "ReportGen",
    probability: 0.55,
    etaHours: 264,
    severity: "info",
