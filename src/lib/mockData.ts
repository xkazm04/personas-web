...
export const MOCK_EVENTS: PersonaEvent[] = [
  ...
  // operator is expected to resolve by hand (retry or discard) until the lane
  // reads empty. Deliberately varied — a timeout, a bad secret, an oversized
  // payload, a vanished target — so the surface looks like a real backlog.
  {
    id: id("ev", 9),
    projectId: "mock-project",
    ...
    sourceType: "slack",
    sourceId: "C04KJ8ZQ2",
    payload: JSON.stringify({ channel: "#support-escalations", user: "U03PLM4", text: "customer on ent-plan reports checkout 500s since 14:02 UTC", ts: "1717070520.418" }, null, 2),
    targetPersonaId: id("p", 2),
    status: "dead_letter",
    errorMessage: "Handler timed out after 30000ms — persona never acknowledged the delivery",
    processedAt: ago(38),
    useCaseId: null,
    createdAt: ago(41),
  },
  {
    id: id("ev", 10),
    projectId: "mock-project",
    eventType: "dependency_update",
    sourceType: "dependabot",
    sourceId: "acme/backend",
    payload: JSON.stringify({ package: "urllib3", from: "2.2.1", to: "2.2.3", advisory: "GHSA-34jh-p97f-mpxf", severity: "moderate" }, null, 2),
    targetPersonaId: id("p", 4),
    status: "dead_letter",
    errorMessage: "Downstream returned 502 from api-prod on every attempt; failure classified permanent",
    processedAt: ago(96),
    useCaseId: null,
    createdAt: ago(105),
  },
  {
    ...
    eventType: "alert_triggered",
    sourceType: "pagerduty",
    sourceId: "service-worker-eu",
    payload: JSON.stringify({ incident_id: "INC-904", severity: "P3", title: "Queue depth above 5k on worker-eu", service: "worker-eu" }, null, 2),
    targetPersonaId: id("p", 2),
    status: "failed",
    errorMessage: "Rate limited by the PagerDuty API (429). Retry after 60s.",
    processedAt: ago(6),
    useCaseId: null,
    createdAt: ago(7),
  },
  {
    ...
    sourceType: "email",
    sourceId: "support@acme.io",
    payload: JSON.stringify({ from: "ops@northwind.example", subject: "Q3 reconciliation export", attachments: [{ name: "reconciliation-q3.csv", bytes: 18_446_221 }] }, null, 2),
    targetPersonaId: id("p", 5),
    status: "dead_letter",
    errorMessage: "Payload rejected: attachment reconciliation-q3.csv exceeds the 10 MB inline limit",
    processedAt: ago(220),
    useCaseId: null,
    createdAt: ago(221),
  },
  {
    ...
    eventType: "webhook_received",
    sourceType: "stripe",
    sourceId: "we_1QfL2xBillingHook",
    payload: JSON.stringify({ type: "invoice.payment_failed", data: { object: { customer: "cus_QhT2", amount_due: 24_900, currency: "usd" } } }, null, 2),
    targetPersonaId: id("p", 5),
    status: "dead_letter",
    errorMessage: "Signature verification failed: no webhook secret configured for endpoint we_1QfL2x",
    processedAt: ago(310),
    useCaseId: null,
    createdAt: ago(310),
  },
  {
    ...
    sourceType: "scheduler",
    sourceId: id("t", 1),
    payload: JSON.stringify({ schedule: "*/15 * * * *", timezone: "Europe/Prague" }),
    targetPersonaId: id("p", 3),
    status: "failed",
    errorMessage: "Target persona is paused; delivery deferred until it is resumed",
    processedAt: ago(11),
    useCaseId: null,
    createdAt: ago(12),
  },
  {
    ...
    sourceType: "jira",
    sourceId: "ACME-2214",
    payload: JSON.stringify({ issue: { key: "ACME-2214", summary: "Checkout retries exhaust the payment token", priority: "High", reporter: "m.novak" } }, null, 2),
    targetPersonaId: id("p", 4),
    status: "dead_letter",
    errorMessage: "Target persona no longer exists — it was deleted after the subscription was created",
    processedAt: ago(480),
    useCaseId: null,
    createdAt: ago(482),
  },
