      ...
      <head>
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: `(function(){try{var T=['dark-midnight','dark-cyan','dark-bronze','dark-frost','dark-purple','dark-pink','dark-red','dark-matrix','light','light-ice','light-news'];var L=['light','light-ice','light-news'];var R=['ar'];var el=document.documentElement;var raw=null;try{raw=localStorage.getItem('personas-theme');}catch(e){}var id;if(raw){try{var p=JSON.parse(raw);if(p&&p.state&&T.indexOf(p.state.themeId)!==-1)id=p.state.themeId;}catch(e){}}if(!id){id=T[Math.floor(Math.random()*T.length)];try{localStorage.setItem('personas-theme',JSON.stringify({state:{themeId:id},version:0}));}catch(e){}}if(id!=='dark-midnight')el.setAttribute('data-theme',id);if(L.indexOf(id)===-1)el.classList.add('dark');else el.classList.remove('dark');var lr=null;try{lr=localStorage.getItem('personas-i18n-storage');}catch(e){}var lang='en';if(lr){try{var lp=JSON.parse(lr);if(lp&&lp.state&&typeof lp.state.language==='string')lang=lp.state.language;}catch(e){}}el.setAttribute('lang',lang);el.setAttribute('data-lang',lang);if(R.indexOf(lang)!==-1)el.setAttribute('dir','rtl');else el.removeAttribute('dir');}catch(e){}})();`,
          }}
        />
