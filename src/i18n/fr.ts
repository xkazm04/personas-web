...
export const fr: Translations = {
    ...
    groups: {
      'agents-prompts': {
        ...
        concepts: [
          ...
          'Modes de prompt structuré + simple',
  ...
  roadmapSection: {
    ...
    gradient: 'produit',
    description: 'Où en est aujourd’hui chaque domaine de Personas — la progression de gauche à droite, pas des promesses de haut en bas.',
    progress: {
    ...
    areas: {
      i18n: { title: 'Internationalisation', caption: '{count} langues, traduites à la main — chaque drapeau se révèle avec la couverture' },
      devices: { title: 'Prise en charge des appareils', caption: 'Personas sur chacune de vos machines' },
      collaboration: { title: 'Collaboration', caption: 'D’un seul opérateur à toute l’organisation' },
      platform: { title: 'Plateforme principale', caption: 'Mode dev, exécution cloud, connecteurs, installations sans effort' },
      templates: { title: 'Galerie de modèles', caption: 'Agents de départ par catégorie — décomptes en direct de la galerie' },
    },
    bars: {
      europe: 'Europe',
  ...
  featureVoting: {
    ...
    features: {
      ...
      i18n: {
        ...
        description: 'Instructions d’agents multilingues, interface localisée et planification adaptée à la région pour les équipes du monde entier.',
      },
      dashboard: {
  ...
  tour: {
    ...
    step3: 'Un agent n’est utile qu’à hauteur des moments pour lesquels il se réveille. Personas peut être déclenché de dix façons — selon un horaire, par un événement, en interrogeant une source ou via un webhook entrant. L’orchestrateur dirige chaque signal vers le bon agent et garde tout en mouvement, se réparant lui-même si une étape échoue.',
    ...
    dashboardHome: 'Bienvenue au centre de contrôle — toute votre flotte sur un seul écran. En haut, les constantes vitales : taux de réussite, exécutions en cours, agents actifs, alertes ouvertes et revues qui vous attendent. En dessous, l\'optimiseur fait remonter une amélioration à fort levier à la fois — en ce moment, un changement de routage qui réduit les coûts sans toucher à la qualité. Les deux panneaux du dessous suivent la santé de chaque agent et les nouvelles mémoires qu\'ils ont apprises et souhaitent promouvoir. Puis l\'image en direct : chaque exécution dès qu\'elle arrive à gauche, quatorze jours de trafic et d\'erreurs à droite. La carte de chaleur montre les exécutions par agent, jour après jour, et la rangée du bas complète le tout — vos meilleurs agents, les prochaines routines planifiées et chaque rotation d\'identifiants. Une page, toute l\'opération.',
    dashboardAgents: 'Voici votre effectif. Chaque carte est une persona — un agent unique doté d\'une seule identité et d\'un ensemble de compétences qu\'il peut composer. Le portrait est généré pour correspondre à son caractère ; en dessous, les statistiques en direct : taux de réussite, exécutions et dépenses. Cliquez sur Exécuter pour en lancer un à la demande, ou ouvrez Détails pour inspecter sa configuration et son historique récent. Cinq agents ici, chacun accomplissant discrètement sa tâche unique.',
    ...
    dashboardEvents: 'Les agents ne travaillent pas en vase clos — ils réagissent à des événements. Voici le bus d\'événements : chaque signal qui circule dans le système, des plannings et webhooks aux messages entre agents. Chaque ligne indique le type d\'événement, sa source, son statut et depuis combien de temps il s\'est déclenché. Les événements échoués peuvent être relancés sur place, et les événements liés s\'enchaînent pour que vous suiviez une seule cascade de bout en bout.',
    dashboardReviews: 'Certaines décisions exigent un humain. Lorsqu\'un agent rencontre quelque chose qu\'il ne devrait pas trancher seul, il s\'arrête et achemine le cas ici. Chaque élément porte la persona, le contexte et l\'action qu\'il propose — approuvez, rejetez ou reportez à plus tard, par clic ou au clavier. Rien de risqué n\'est publié sans votre validation, et la file maintient le reste de la flotte en mouvement pendant que vous décidez.',
