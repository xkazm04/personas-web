...
## Target Context
- Context ID: ctx_1776164915188_4gzhnzf
- Context Name: Conversion & Onboarding Sections
...
**Context Description**:
Pricing tiers, get-started onboarding flow, FAQ, CTAs, footer, and how-it-works page that guide visitors toward conversion

**Files in this Context** (25 files):
- src/components/sections/Pricing.tsx
- src/components/sections/pricing/index.tsx
- src/components/sections/pricing/data.ts
- src/components/sections/pricing/FeatureGroupCard.tsx
- src/components/sections/GetStarted.tsx
- src/components/sections/get-started/index.tsx
- src/components/sections/get-started/data.ts
- src/components/sections/get-started/StepChip.tsx
- src/components/sections/get-started/StepContent.tsx
- src/components/sections/get-started/visuals/ConnectVisual.tsx
...
- src/components/sections/get-started/visuals/ImproveVisual.tsx
- src/components/sections/get-started/visuals/WorkVisual.tsx
- src/components/sections/get-started/visuals/index.ts
- src/components/sections/get-started/visuals/types.ts
...
Found 16 active idea(s) (25 total, excluding rejected/implemented for brevity):

### Pending Ideas (15)
1. **FAQ keyboard nav focuses wrong item across columns** (code_quality)
   - FAQ.tsx splits items into two columns but keyboard navigation (ArrowUp/ArrowDown) uses a single flat index via modulo arithmetic. Left column items 0-...
2. **GetStarted parallel arrays lack bounds guard** (code_quality)
   - In get-started/index.tsx, four independent arrays (TOUR_STEPS, STEP_VISUALS, STEP_ICONS, STEP_BRAND) are indexed by the same active value with zero bo...
3. **Add aria-live and alt text to GetStarted step visuals** (ui)
   - The GetStarted step carousel auto-advances every 7 seconds and swaps between 5 visual components (DownloadVisual, ConnectVisual, CreateVisual, WorkVis...
4. **Respect prefers-reduced-motion across all sections** (ui)
   - The conversion sections use extensive Framer Motion animations (fadeUp stagger, orbital ring rotation at 40s, step glow pulses, auto-advancing carouse...
5. **Replace jargon with plain language + tooltips** (ui)
   - Audit GetStarted step descriptions, Pricing feature bullets, and FAQ answers to replace technical jargon with plain language. Change "breeding cycle" ...
6. **Add platform timeline and expectations to waitlist** (user_benefit)
   - Update the macOS and Linux platform pills and WaitlistModal to include an estimated timeline (e.g. "Expected Q3 2026") and a brief explanation of what...
7. **Add prerequisite checklist before download CTA** (user_benefit)
   - Add a visible prerequisites callout directly in DownloadCTA.tsx and MidPageCTA.tsx that explains users need Claude CLI and an Anthropic subscription b...
8. **Proactive objection surfacing based on scroll behavior** (user_benefit)
   - Track scroll depth and hover dwell time on the DownloadCTA section. When a user scrolls to the download area but does not click within 5 seconds (hesi...
9. **Smart feature group prioritization from UTM context** (functionality)
   - Use UTM parameters, referrer, and an optional one-click role badge to reorder the 6 feature groups in the Pricing section so the most relevant capabil...
10. **AI connector recommendation based on use case input** (user_benefit)
   - In the GetStarted Step 2 (Connect Your Tools), add a small text input where users describe their intended use case in natural language (e.g., "I want ...
11. **Role-aware onboarding with adaptive step content** (functionality)
   - Add a brief role-selection prompt (Developer / Product Manager / Team Lead) at the top of the GetStarted section that swaps step descriptions, example...
12. **Semantic FAQ search with natural language matching** (functionality)
   - Add a search input above the FAQ accordion that uses text embeddings to match free-form user questions to the 6 existing FAQ items. When the user type...
13. **Add inline download CTA to feature group cards** (functionality)
   - FeatureGroupCard currently ends with a 'Read the guide' link, which is informational but not conversion-oriented. Add a secondary CTA below it � a com...
14. **Add platform-aware CTA with fallback for non-Windows users** (user_benefit)
   - DownloadCTA and the onboarding DownloadVisual prominently show macOS and Linux as 'Coming soon' with no timeline or alternative. Detect the visitor's ...
15. **Show step labels on mobile GetStarted chips** (user_benefit)
   - The StepChip component hides step title text on screens below sm breakpoint via 'hidden sm:inline', leaving mobile users with only a number and icon t...

### Accepted Ideas (1)
1. **FAQ illustration array out-of-bounds crash** (code_quality)
   - In FAQ.tsx, illustrations[i] is accessed without bounds checking against a hardcoded 6-element array. If translations add a 7th+ FAQ question, the ill...
...

{
  ...
  "project_id": "39de516e-059b-4b1a-86ad-f28e8abddaf7",
  "context_id": "ctx_1776164915188_4gzhnzf",
  "scan_type": "brand_artist",
  "category": "<category>",
  "title": "<title>",
