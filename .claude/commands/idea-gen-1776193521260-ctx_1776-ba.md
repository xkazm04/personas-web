...
## Target Context
- Context ID: ctx_1776164949805_03b37lr
- Context Name: Guide Content & Rich Blocks
...
**Context Description**:
Ten guide content modules (agents, credentials, deployment, etc.) and eleven rich block components (architecture diagrams, code compare, step wizard, etc.) for visual documentation

**Files in this Context** (23 files):
- src/components/guide/GuideBlocks.tsx
- src/components/guide/blocks/index.ts
- src/components/guide/blocks/ArchitectureDiagram.tsx
- src/components/guide/blocks/Callout.tsx
- src/components/guide/blocks/Checklist.tsx
- src/components/guide/blocks/CodeCompare.tsx
- src/components/guide/blocks/CompareBlock.tsx
- src/components/guide/blocks/FeatureHighlight.tsx
- src/components/guide/blocks/KeyboardGrid.tsx
- src/components/guide/blocks/MarkdownTable.tsx
- src/components/guide/blocks/StepWizard.tsx
- src/components/guide/blocks/UseCaseGrid.tsx
- src/data/guide/content/index.ts
- src/data/guide/content/agents-prompts.ts
- src/data/guide/content/credentials.ts
- src/data/guide/content/deployment.ts
- src/data/guide/content/getting-started.ts
- src/data/guide/content/memories.ts
- src/data/guide/content/monitoring.ts
- src/data/guide/content/pipelines.ts
- src/data/guide/content/testing.ts
- src/data/guide/content/triggers.ts
- src/data/guide/content/troubleshooting.ts
...
Found 7 active idea(s) (15 total, excluding rejected/implemented for brevity):

### Pending Ideas (7)
1. **Add expand/collapse to CodeCompare blocks with syntax-highlighted line diffing** (ui)
   - CodeCompare.tsx renders before/after code in a side-by-side grid, but long code blocks force excessive vertical scrolling and the plain-text rendering...
2. **Redesign ArchitectureDiagram for responsive vertical stacking with animated flow** (ui)
   - ArchitectureDiagram.tsx uses flex-wrap with horizontal ArrowRight icons between nodes. On narrow viewports, when nodes wrap to the next line, the righ...
3. **Add active-step tracking and completion state to StepWizard** (user_benefit)
   - The StepWizard block (StepWizard.tsx) renders numbered steps with a vertical connecting line but has no concept of current step, completed steps, or p...
4. **Responsive card layout for tables on mobile** (ui)
   - Replace the horizontal-scrolling table layout on mobile with a stacked card view where each row becomes a card with header labels above each value. Th...
5. **CodeCompare inline diff highlighting** (functionality)
   - Enhance the CodeCompare block to highlight added and removed lines with green and red background tinting, similar to GitHub diff views. Currently the ...
6. **Guide progress breadcrumb with completion tracking** (ui)
   - Add a breadcrumb bar and lightweight progress indicator to the guide layout showing the user where they are within a category (e.g., Getting Started >...
7. **Interactive checklist with progress persistence** (ui)
   - Transform the static Checklist component into an interactive one with clickable checkboxes that persist state via localStorage. Users currently see a ...
...

{
  ...
  "project_id": "39de516e-059b-4b1a-86ad-f28e8abddaf7",
  "context_id": "ctx_1776164949805_03b37lr",
  "scan_type": "brand_artist",
  "category": "<category>",
  "title": "<title>",
