...
## Target Context
- Context ID: ctx_1776165032285_fns1ein
- Context Name: Roadmap, Changelog & Feedback
...
**Context Description**:
Product roadmap with phase cards, changelog with release notes, feature voting with comments and boost buttons, and use-case/download pages with OS detection

**Files in this Context** (33 files):
- src/app/roadmap/page.tsx
- src/components/sections/Roadmap.tsx
- src/components/sections/roadmap/index.tsx
- src/components/sections/roadmap/data.ts
- src/components/sections/roadmap/types.ts
- src/components/sections/roadmap/components/RoadmapCard.tsx
- src/components/sections/roadmap/components/RoadmapProgress.tsx
- src/app/changelog/page.tsx
- src/app/changelog/layout.tsx
- src/components/sections/Changelog.tsx
- src/components/sections/FeatureVoting.tsx
- src/components/sections/feature-voting/index.tsx
- src/components/sections/feature-voting/data.ts
- src/components/sections/feature-voting/types.ts
- src/components/sections/feature-voting/local-types.ts
- src/components/sections/feature-voting/VoteCard.tsx
- src/components/sections/feature-voting/VoteParticles.tsx
- src/components/sections/feature-voting/CustomRequest.tsx
- src/components/sections/feature-voting/NotifyInput.tsx
- src/components/sections/feature-voting/components/FeatureVoteCard.tsx
- src/components/sections/feature-voting/components/FeatureBoostButton.tsx
- src/components/sections/feature-voting/components/FeatureVoteIllustration.tsx
- src/components/sections/feature-voting/components/CommentThread.tsx
- src/components/sections/feature-voting/components/CommentBubble.tsx
- src/components/sections/feature-voting/components/CommentInput.tsx
- src/components/sections/feature-voting/components/CustomFeatureRequest.tsx
- src/app/api/votes/route.ts
- src/app/api/votes/rate-limit.ts
- src/app/api/votes/storage.ts
- src/app/api/feature-requests/route.ts
- src/app/api/roadmap/route.ts
- src/data/roadmap-phases.ts
...
Found 7 active idea(s) (10 total, excluding rejected/implemented for brevity):

### Pending Ideas (7)
1. **Add changelog type filters and sticky nav** (functionality)
   - The changelog currently renders all releases in a single vertical list with no way to filter or navigate. Add a sticky filter bar below the highlights...
2. **Unify animation timing into a consistent system** (ui)
   - Animation durations are scattered across components with no coherent timing system: FeatureVoteCard expand uses 0.3s, comments reveal uses 0.25s, boos...
3. **Celebrate shipped features users voted for** (user_benefit)
   - When a feature ships that a user previously voted for, the current UI shows a muted shipped badge and a generic message. Transform this into a celebra...
4. **Make roadmap progress dynamic with last-updated timestamps** (ui)
   - The RoadmapProgress component shows a hardcoded 73% and 11/15 phases that never update, creating distrust when users return and see identical numbers....
5. **Close the feature-request feedback loop with status tracking** (functionality)
   - When users submit a custom feature request via CustomFeatureRequest.tsx, they see a 3-second checkmark and then nothing � a dead end that feels like s...
6. **Add changelog search and breaking-change filter** (ui)
   - Users currently must scroll through every release to find relevant changes, creating significant friction for anxious users worried about breaking cha...
7. **Replace demo-mode disclaimer with trust-building transparency** (user_benefit)
   - The current feature voting section shows a vague demo mode disclaimer that erodes user trust after they invest time voting and commenting. Replace it ...
...

{
  ...
  "project_id": "39de516e-059b-4b1a-86ad-f28e8abddaf7",
  "context_id": "ctx_1776165032285_fns1ein",
  "scan_type": "brand_artist",
  "category": "<category>",
  "title": "<title>",
