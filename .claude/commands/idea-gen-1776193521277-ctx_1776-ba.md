...
## Target Context
- Context ID: ctx_1776164942379_xpx1fc5
- Context Name: Guide Navigation & Search
...
**Context Description**:
Guide routing, sidebar navigation, fuzzy search with combobox, category pages, and topic page rendering for the documentation system

**Files in this Context** (18 files):
- src/app/guide/page.tsx
- src/app/guide/layout.tsx
- src/app/guide/[category]/page.tsx
- src/app/guide/[category]/CategoryTopics.tsx
- src/app/guide/[category]/opengraph-image.tsx
- src/components/guide/GuideSidebar.tsx
- src/components/guide/SearchCombobox.tsx
- src/components/guide/RelatedTopics.tsx
- src/components/guide/ModuleBadge.tsx
- src/components/guide/GuideMarkdown.tsx
- src/data/guide/categories.ts
- src/data/guide/topics.ts
- src/data/guide/types.ts
- src/data/guide/desktop-modules.ts
- src/data/guide/illustrations.ts
- src/lib/guide-link.ts
- src/lib/guide-search.ts
- src/lib/guide-utils.ts
...
Found 6 active idea(s) (15 total, excluding rejected/implemented for brevity):

### Pending Ideas (6)
1. **Unify search behavior across all three surfaces** (ui)
   - Standardize the three search implementations (SearchCombobox 2-char min + fuzzy + 150ms debounce, GuideSidebar 1-char + substring, CategoryTopics 0-ch...
2. **Previous/next topic navigation with progress bar** (functionality)
   - Add a bottom navigation bar to topic detail pages showing previous and next topics within the same category, styled as two card-like buttons with topi...
3. **Enriched empty states with smart suggestions** (ui)
   - Replace the bare "No topics found" messages across all three search surfaces (SearchCombobox, GuideSidebar, CategoryTopics) with contextual empty stat...
4. **Breadcrumb navigation with you-are-here context** (functionality)
   - Add a breadcrumb bar above topic content showing the full navigation path (e.g., Guide > Agents and Prompts > Creating a New Agent). Include the curre...
5. **Guided reading paths for new users** (functionality)
   - Add a Getting Started Path that sequences 5-7 essential topics in reading order, displayed as a numbered progress track on the guide landing page and ...
6. **Empathetic zero-results search with suggestions** (ui)
   - When SearchCombobox returns no results, replace the bare No topics found message with contextual help: suggest related categories, show popular topics...
...

{
  ...
  "project_id": "39de516e-059b-4b1a-86ad-f28e8abddaf7",
  "context_id": "ctx_1776164942379_xpx1fc5",
  "scan_type": "brand_artist",
  "category": "<category>",
  "title": "<title>",
